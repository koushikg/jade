...
Parser.prototype = {
  ...
  addSrcFmId: function (attrs, fmId) {
    if (!attrs || !fmId) {
      return;
    }
    var src_attrs = attrs.filter(a => a.name == 'src' || a.name == 'href');
    if (!src_attrs) {
      return;
    }
    src_attrs.forEach(a => {
      ...
      val += (val.includes('?') ? '&' : '?') + `fmId=${fmId}`;
      if (postHash) {
        val += '#' + postHash;
      }
      if (start_with_qoutes) {
        val = `${start_with_qoutes}${val}${start_with_qoutes}`;
      }
      console.info(`koushik: before(${a.val}), after(${val})`);
      a.val = val;
    });
  },
      ...
      while (true) {
        switch (this.peek().type) {
          ...
          case 'attrs':
            ...
            var tok = this.advance();
            var attrs = tok.attrs;
            if (process && process.env && process.env.SRC_FM_ID) {
              this.addSrcFmId(attrs, process.env.SRC_FM_ID);
            }
